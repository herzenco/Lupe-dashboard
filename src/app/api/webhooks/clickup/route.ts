  ...
  try {
    ...
    switch (event) {
        ...
        if (history_items && history_items.length > 0) {
          for (const item of history_items) {
            ...
            if (field === "name") {
              await supabase
                .from("tasks")
                .update({ name: after, updated_at: new Date().toISOString() })
                .eq("id", task_id);
      ...
      case "taskStatusUpdated": {
        if (history_items?.[0]) {
          const newStatus =
            history_items[0].after?.status || history_items[0].after;
          await supabase
            .from("tasks")
            .update({
              status: newStatus,
          ...
          await supabase.from("task_comments").upsert(
            {
              ...
              task_id,
              user_name: comment.user?.username || "unknown",
              comment_text:
      ...
      case "taskPriorityUpdated": {
        if (history_items?.[0]) {
          const newPriority =
            history_items[0].after?.orderindex ?? history_items[0].after;
          await supabase
            .from("tasks")
            .update({
